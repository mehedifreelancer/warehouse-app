...
const ProcessingOrders = () => {
  ...
  const fetchPendingOrders = async (page: number = 1) => {
    setLoading(true);
    try {
      const ordersRes = await getOrders(page, "processing");
      ...
      {loading ? (
        ...
        >
          ...
          <Column
            ...
            body={(rowData) => (
              <div className="flex gap-2">
                <Button className="btn-success text-xl">Ready to Ship </Button>
                <Button className="btn-gray text-xl">Print Invoice </Button>
              </div>
            )}
          />
        </DataTable>
